...
**Status:** GENERATED as of 2026-08-04 from the checked-in stylesheet by
`tests/test_ui_contrast.py`. **68 pairs, 0 failures.** Regenerate with:

```sh
python -m tests.test_ui_contrast --update
```

The table below is machine-written between the generated markers — do not
hand-edit it. `tests/test_ui_contrast.py::test_published_contrast_table_is_current`
fails if it drifts from the stylesheet.

## Headline

| | |
|---|---|
| Pairs measured | **68** (34 light, 34 dark) |
| Failures | **0** |
| Tightest pair | light `--border` on `--bg` — **3.312:1** against a 3:1 floor |
| Tightest text pair | dark `.status-badge--down` — **4.832:1** against 4.5:1 |
| Previously-shipped AA failures closed | **1** (`.skip-link:focus-visible`, 2.526:1 → 7.199:1) |
| Previously-shipped SC 1.4.11 failures closed | **2** (light `--border` 1.342:1 → 3.312:1; `.status-badge--ops-warn` border 2.414:1 → 6.357:1) |
...
|---|---|---|---|
| 1 · button ground | `button, .button` | 6.583:1 vs `#fff` | 7.199:1 vs `var(--bg)` |
| 1b · hover ground | `button:hover` | 5.037:1 | 8.264:1 |
| 2 · focus ring vs `--bg` | `:focus-visible` | 6.198:1 | 7.199:1 |
| 2 · focus ring vs `--card-bg` | `:focus-visible` | 6.372:1 | 6.584:1 |
| 3 · link | `.breadcrumb a` | 6.198:1 | 7.199:1 |
| 4 · skip-link ground | `.skip-link:focus-visible` | 6.583:1 | 7.199:1 |
| 5 · badge-flash tint | `@keyframes badge-flash` | see below | see below |

**Role 5 is deliberately not assigned a ratio.** It is a transient,
30%-opacity `color-mix(… , transparent)` overlay animated over a status
pill that is already independently legible, and it is gated behind
`prefers-reduced-motion: no-preference`. WCAG does not crisply mandate a
ratio for that case, and inventing one would be exactly the kind of
hand-asserted number this document exists to eliminate. The pill's own
text/background pair is measured (rows below) and carries the information.
...
2. **`.card .note` lost headroom**, 5.025:1 → 4.886:1, because light
   `--card-bg` is no longer pure white. It is an untouched v1 literal
   (`#6f6f6f`) and still clears 4.5:1.
