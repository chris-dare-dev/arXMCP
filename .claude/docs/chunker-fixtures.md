...
`theorem_label` correctness is **not** covered by the golden suite: `expected.json` pins only `chunk_count` / `kind_counts` / `expected_chunk_ids`, and `_compute_chunk_id` is a hash of `paper_id + preamble_text + body_text` — independent of `theorem_label`. A theorem-label regression is therefore invisible to `TestFixtureSuite`. Label extraction (`_extract_theorem_label`) is instead locked by direct-assertion tests: `TestTheoremLabelExtraction` (unit) and `TestSectionlessAutoIdLabel` (`_chunks_from_html` → assert `.theorem_label`, the regression guard for section-less LaTeXML auto-ids like `Thmdefix1` that `_AUTO_ID_RE` must classify as auto, not as a `\label{}` key). When changing label classification, update those tests — a new golden fixture would not detect the change.
