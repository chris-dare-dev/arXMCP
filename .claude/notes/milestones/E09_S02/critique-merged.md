...
- F1 — fixed (HIGH; `_merge_paper_inspire`'s `ON MATCH SET` now uses
  `COALESCE($new, p.col)` so a re-MERGE with NULL preserves the
  prior value). Regression guards:
  `tests/test_inspire_ingest.py::TestE09S02RectificationGuards::test_f1_inspire_remerge_preserves_doi_when_response_drops_field`
  + `::test_f1_inspire_merge_overwrites_with_new_non_null`.
- F2 — fixed (HIGH; `enrich()` and `_fetch_inspire_record` now use
  the new `_validate_paper_id_either_style` validator that delegates
  to `ingest.identifiers.is_valid_paper_id`, which accepts old-style
  arXiv IDs like `hep-th/9711200`). Regression guards:
  `::test_f2_enrich_accepts_old_style_hep_th_id`,
  `::test_f2_validator_rejects_truly_invalid_ids`.
- F3 — addressed (MEDIUM, documentation choice per the critic's "or
  document" option). Reclassification semantics now explicitly
  documented at the gate-fail branch: a re-classification does NOT
  retract earlier curation. The previously-stamped INSPIRE columns
  survive in the graph, matching the brief's "additive enrichment"
  framing.
- F4 — fixed (MEDIUM; `in_corpus = _existing_paper_ids(conn)` is
  refreshed AFTER Pass 1 so newly-created nodes from
  `_merge_paper_inspire` are visible to the citation-pass filter).
  Regression guard:
  `::test_f4_inspire_creates_node_then_emits_edge_to_resolved_paper`.
- F5 — fixed (MEDIUM; `_fetch_inspire_record` now calls
  `_validate_paper_id_either_style` as its first statement —
  defense-in-depth for Threat 1 if a future caller bypasses
  `enrich()`). Regression guard:
  `::test_f5_fetch_inspire_record_validates_paper_id`.
- F6 — fixed (MEDIUM; the `URLError` catch in the resolution-pass
  loop now increments `new_in_pass` and flushes the checkpoint at
  batch boundaries, so a long failure run is no longer fragile to
  hard kill). Regression guard:
  `::test_f6_failure_run_flushes_checkpoint_at_batch_size`.
- F7 — fixed (MEDIUM; dropped the `--source` CLI flag entirely
  along with the `_normalize_source` helper. The script's identity
  IS INSPIRE — a flag advertising selectivity it didn't deliver
  was a foot-gun. Tests for `_normalize_source` removed
  accordingly).
- F8 — fixed (MEDIUM; added a forward-compat anchor test that
  pre-populates a paper with a real arXiv-style category
  (`p.categories = 'hep-th'`) and verifies `enrich()` reaches it,
  regardless of whether the iteration filter ever respects the
  `categories` column). Regression guard:
  `::test_f8_arxiv_categories_filter_anchor_for_post_f9`.
- F9 — deferred (LOW; source-string casing inconsistency between
  OpenAlex's camelCase `"openAlex"` and INSPIRE's lowercase
  `"inspire"`. Documented; a future cleanup PR can normalize the
  OpenAlex side).
- F10 — deferred (LOW; `enrich()` vs `ingest()` naming
  inconsistency. Bikeshedding; revisit after E09_S03 lands and we
  see the third ingest-shape function).
- F11 — closed structurally (LOW; was about `_normalize_source`
  duplication. Dropping the helper for F7 also removes the
  duplication. No regression test needed because the duplicate
  helper no longer exists).

**Invalidation rate:** 0 / 2 HIGH findings invalidated (0%). Both
HIGHs re-verified against the cited file:line ranges; F2 was
empirically reproduced by reading the regex.

**Test count delta:** 1248 → 1253 passed, 4 skipped (+8 new
rectification guards, −3 removed `_normalize_source` tests).
