...
- F1 — fixed: `chunk_textbook_markdown` wraps IO+impl in `PER_PAPER_FAILURE_EXCEPTIONS`
  -> logs + returns `[]` (mirrors `chunk_textbook`), so a missing markdown no longer
  aborts a batch. Regression: `test_missing_markdown_returns_empty_not_raises`.
- F2 — fixed: `test_chunker_flag_routes_to_correct_chunker` monkeypatches both chunkers
  and asserts `ingest_textbook_paper(chunker=...)` routes correctly (markdown + default).
- F3 — fixed: `_inline_math_open` strips escaped `\$` before the parity count; the
  `$$`-merge run is capped at `_MAX_MERGE_BLOCKS=50` so a stray `$` cannot swallow the
  document. Regressions: `test_escaped_dollar_not_treated_as_open`,
  `test_merge_cap_bounds_blast_radius`. (Full code-fence stripping deferred — bounded by
  the cap; rare in math.AG/NT MinerU output.)
- F4 — fixed: the section breadcrumb is folded into the `chunk_id` hash (via the
  preamble slot), so identical bodies in DIFFERENT sections are both kept (no
  cross-section content loss / mis-attribution); same-section identical bodies still
  dedup. Regressions: `test_identical_body_distinct_sections_both_kept`,
  `test_identical_body_same_section_deduped`. (On real segments this recovered chunks:
  p001-020 20->43, p101-120 18->31.)
- F5 — fixed: markdown read with `errors="strict"`; `UnicodeDecodeError` (a `ValueError`)
  is caught by the F1 envelope as a logged per-paper failure instead of silent U+FFFD.
  Regression: `test_non_utf8_markdown_returns_empty_not_raises`.
- F7 — fixed: the empty-result warning in `notebook_textbook_ingest` now names the
  chunker-appropriate artifact (MinerU markdown vs LaTeXML HTML).
- F6 — deferred (LOW): glob symlink-containment recheck. Matches the existing HTML-path
  posture (`_textbook_html_path`); loopback-only local-operator threat, paper_id already
  traversal-validated.

Re-verify: 0 findings invalidated; all 5 MEDIUM confirmed by the adversary's live probes.
