...
Re-verify ran for all 8 findings. None invalidated; every cited
file:line region matched the critique's "what" claim. F1
reproduced live against SQLite 3.51 (NUL byte → OperationalError).

- **F1 (HIGH) — fixed.** `fts5_phrase_quote` now strips control
  characters (`\x00-\x1f`) BEFORE wrapping the literal in quotes.
  Regression tests
  `TestFts5PhraseQuote::test_strips_nul_byte`,
  `test_strips_control_chars`, plus
  `test_fts5_phrase_quote_blocks_injection` extended with a NUL
  payload all verify the boundary guard.
- **F2 (HIGH) — fixed.** `index_theorem_names_for_paper` now
  predicate-pushes the paper_id filter via
  `.search().where(..., prefilter=True).to_arrow()` with a
  defensive fallback to a full `to_arrow()` for test fixtures
  that pass plain Arrow tables.
  `index_theorem_names_all_papers` materializes the chunks table
  exactly ONCE and groups in Python — was N+1 reads, now 1 read.
- **F3 (MEDIUM) — fixed.** `_in_memory_scan_fallback` synthesizes
  `dedup_key` via the same recipe as the indexer. Regression
  test extended to assert `dedup_key is not None` and 16-char
  length.
- **F4 (MEDIUM) — fixed.** New `retrieval_mode` value
  `"empty_after_normalization"` distinguishes empty-input from
  empty-result. Test renamed +
  `test_all_punctuation_name_returns_empty_with_distinct_mode`
  verifies both punctuation-only AND whitespace-only inputs.
  **No hash repin required**: `retrieval_mode` is a response
  field, not part of `tools/list`'s input/output schema; the
  taxonomy expands purely additively.
- **F5 (MEDIUM) — fixed.** Schema-drop log bumped from INFO to
  WARNING with an explicit re-run instruction.
  `Resources.startup` logs the row count at open time and emits
  a WARNING when the store is empty.
- **F6 (MEDIUM) — deferred.** Path validator on
  `theorem_names_db_path` / `cache_db_path` is a real hardening
  opportunity but extends beyond E10_S02 and affects every
  `Config` path field. Tracked for E13 (security hardening).
- **F7 (LOW) — deferred.** `normalize_name` over-collapses
  K(π,1)/K1 and L²/L2 — but the `dedup_key` uses raw
  `theorem_name`, so the over-matching is result-set noise, not
  data loss. v1 ships the aggressive recipe.
- **F8 (LOW) — deferred per critic.** Test infrastructure
  ergonomics; doesn't affect production. Same pattern in
  E10_S01/S03; worth revisiting if pytest-asyncio is adopted.

**Invalidation rate:** 0 / 8 findings invalidated (0%). Adversary
critic was well-calibrated.

**Test count delta after rectify:** 1425 passing (+2 regression
guards from post-implement 1423). 4 skipped, 0 failed, ruff clean.

| Finding | Severity | Status |
|---|---|---|
| F1 — NUL byte in phrase-quote | HIGH | fixed (regression test) |
| F2 — N+1 LanceDB scans | HIGH | fixed (predicate-push) |
| F3 — fallback path dedup_key=None | MEDIUM | fixed (regression test) |
| F4 — misleading fts5_exact mode tag | MEDIUM | fixed (new mode tag) |
| F5 — silent schema-drop | MEDIUM | fixed (WARN log + startup row count) |
| F6 — path validator | MEDIUM | deferred (E13 scope) |
| F7 — normalize over-collapses | LOW | deferred (documentation) |
| F8 — test event-loop ergonomics | LOW | deferred per critic |
