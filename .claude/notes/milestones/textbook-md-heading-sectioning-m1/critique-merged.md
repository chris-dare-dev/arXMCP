...
- F1 — fixed: added `"$": "\\$"` to `_HEADING_PROSE_ESCAPE`
  (`ingest/textbook_renderer.py`). Regression guard:
  `tests/test_textbook_renderer.py::TestBuildLatexWrapper::test_unbalanced_dollar_in_heading_escaped`.
- F2 — fixed: subsumed by F1's escape; `_MATH_SPAN_RE` comment amended to state an
  unbalanced `$` is neutralized by escaping (not by the regex alone). Regression
  guard: `test_unbalanced_dollar_then_special_char_escaped`.
- F3 — fixed: added `test_unbalanced_dollar_in_heading_escaped`,
  `test_unbalanced_dollar_then_special_char_escaped`, `test_math_only_heading_title`,
  `test_display_math_in_heading_preserved`.
- F4 — fixed: `_neutralize_structural_commands` docstring corrected to note it counts
  RAW markdown and diverges from the wrapper's post-heading-conversion neutralization.
  (Kept, not deleted — it is a pre-existing observability helper; removal is out of
  this milestone's scope.)
- F5 — deferred (LOW): heading math-span `\input` passthrough is identical to existing
  body-math passthrough, bounded by the loopback-only / no-shell-escape sandbox posture
  (`.claude/docs/security-pdf-sandbox.md`). No code change; accepted posture.

Re-verify gate: F1 (the only HIGH) re-read and confirmed real before fixing
(`_HEADING_PROSE_ESCAPE` lacked `$`). 0 findings invalidated. Adversary invalidation
rate: 0%.
