...
- **F1 (HIGH) — FIXED.** Added `--proc /proc` to the bwrap argv in
  `tools/arxiv_fetch.py::_build_sandbox_cmd`. Closes the documented-
  broken `--unshare-pid` without procfs failure mode. Regression
  guard: extended `test_build_sandbox_cmd_linux_prepends_bwrap` to
  assert `--proc` is paired with `/proc` target.
- **F2 (HIGH) — FIXED.** Added `--dev /dev` to the bwrap argv.
  Without it, Perl helpers + fontconfig + subprocesses redirecting to
  `/dev/null` would all ENOENT inside the sandbox. Regression
  guard: same test now asserts `--dev` is paired with `/dev`.
- **F3 (HIGH) — FIXED via removal.** Dropped the `--timeout=300`
  latexmlc CLI flag and the `LATEXML_INTERNAL_TIMEOUT_SECONDS`
  constant. Took the adversary's option (a) — the safer ship — since
  no live-integration test proves the flag form is accepted by the
  locally-installed LaTeXML. Python-side `subprocess` timeout +
  `killpg` remain the load-bearing timeout discipline. Updated
  `test_parse_with_latexml_threads_sandbox_to_popen` to assert
  the flag is NOT present (regression guard against accidental
  re-add without a live test).
- **F4 (MEDIUM) — FIXED.** Added `start_new_session=True` to the
  `subprocess.run(...)` call in `ops/drift_check.py::render_fixture`
  per synthesis D6. Regression guard:
  `TestProcessGroupKill::test_render_fixture_uses_start_new_session`
  AST guard pins the contract (mirrors the existing
  `test_parse_with_latexml_uses_process_group_kill` pattern;
  AST-based so it runs on Windows too).
- **F5 (MEDIUM) — FIXED.** Replaced the blanket
  `lambda self: True` stub for `Path.is_file` in
  `test_detect_sandbox_layer_darwin_when_present` with a
  discriminating stub that records which paths were checked AND
  asserts both `/usr/bin/sandbox-exec` and `sandbox.sb` were
  exercised. A future refactor that drops either check now fails
  this test.
- **F6 (MEDIUM) — FIXED.** Added `if Path("/lib").exists():`
  guard around the `--ro-bind /lib /lib` pair in the bwrap argv
  builder. Minimal-image (Alpine, scratch) operators no longer
  trip `bwrap: Can't bind /lib` errors. Regression guard:
  `test_build_sandbox_cmd_linux_skips_missing_lib_dirs` mocks
  `Path.exists` to report neither dir exists; asserts the produced
  argv has neither ro-bind pair.
- **F7 (MEDIUM) — FIXED.** Added
  `(allow file-read* (subpath (param "TMPDIR_SUBDIR")))` to
  `infra/latexml/sandbox.sb` so Perl atomic-write patterns
  (write-then-read) inside the per-paper tmp area don't trip the
  sandbox. Technically out of E13_S03b's wiring scope, but the
  cost was 1 line + 1 test assertion. Regression guard:
  `TestSandboxProfile::test_profile_allows_tmpdir_subdir_read`
  pins the literal form.
- **F8 (MEDIUM) — FIXED.** Extended
  `test_parse_with_latexml_threads_sandbox_to_popen` to assert
  the six load-bearing bwrap isolation flags (`--proc`, `--dev`,
  `--unshare-net`, `--unshare-pid`, `--die-with-parent`,
  `--new-session`) ALL appear in the captured argv. A future
  refactor that drops any of them fails this test loudly.
- **F9 (LOW) — DEFERRED.** Style only (`cmd` re-bound inside
  the `with` block). The contract (`_build_sandbox_cmd` does
  not mutate input) is independently pinned by
  `test_build_sandbox_cmd_does_not_mutate_input`. Future style
  sweep.
- **F10 (LOW) — DEFERRED.** Style only (function-local import
  in `render_fixture`). No circular-import workaround required;
  works correctly. Future style sweep.
