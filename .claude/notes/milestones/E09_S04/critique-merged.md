...
- F1 — fixed (HIGH; densified the synthetic graph from
  `EDGES_PER_PAPER = 3` (150 edges) to `EDGES_PER_PAPER = 10`
  (500 edges) to approximate real OpenAlex+INSPIRE citation
  density for math.AG papers. Added a fixture-time assertion
  that the graph has exactly `EXPECTED_EDGE_COUNT = 500` edges
  so a future helper refactor that silently lowers density
  surfaces here, not as a false-pass on the perf gate).
- F2 — fixed (MEDIUM; tightened the `chunk_id=None` doc-pin
  assertion. The previous 3-way disjunction accepted `"deferred
  to E07_S04"` as a substitute for the AC#5 round-budget
  framing. Now requires `"third round"` OR `"exhausts"`
  explicitly).
- F3 — fixed (MEDIUM; `graph_corpus` fixture asserts at setup
  time that the entry paper P_0 directly cites the
  missing-from-LanceDB paper P_{N-1}. A future change to
  `build_synthetic_kuzu_graph`'s edge-construction semantics
  surfaces here rather than as a silent AC#7 pass).
- F4 — fixed (MEDIUM; `fake_resources` fixture now `del`s the
  LanceDB connection AND chunks_table handle in the `finally:`
  block after `reset_resources_for_tests()`. Pre-empts
  GC-ordering flakes on Windows CI in long suites).
- F5 — fixed (MEDIUM; added a "Performance contract" paragraph
  to the `cite_neighbors` docstring pinning the 500ms target
  with cross-references to the perf-gate test and the workflow
  doc. A future refactor that breaks the contract now has an
  in-source signal).
- F6 — deferred (MEDIUM; per-test fixture rebuild. Pure
  performance, not correctness. Suite runtime went from 52s to
  76s after F1 densification; promoting fixture scope to
  session would cut this but adds cross-test coupling. Defer
  until suite-time becomes a real concern).
- F7 — fixed (LOW; tightened `body_text` assertion to
  substring-match the synthetic signature phrase `"Statement of
  theorem for"`. A field-swap regression in the handler now
  surfaces).
- F8 — fixed (LOW; doc-pin assertions normalize whitespace
  before substring-matching, so a markdown reflow that wraps a
  phrase across lines doesn't trip them).
- F9 — fixed (LOW; the doc's worked example splats the
  comprehension into `asyncio.gather(*[...])`. The previous
  generator form was a `TypeError` if copy-pasted by a reader).

**Invalidation rate:** 0 / 1 HIGH findings invalidated (0%). F1
re-verified — the synthetic-vs-real density gap was real and was
correctly diagnosed.

**Test count delta:** 1312 → 1312 passed, 4 skipped (no new tests
added in rectification — the fixes tightened existing assertions
and added fixture-time guards. The +10 test-count delta from the
feat commit is preserved).
