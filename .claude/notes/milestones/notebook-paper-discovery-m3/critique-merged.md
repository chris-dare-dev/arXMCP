...
- F1 (MEDIUM) — fixed in `tools/discover_for_notebook.py`: the dedup `existing_ids`
  set is now built through a `_strip_version` helper (mirrors
  `_arxiv_api.parse_atom_feed`'s `split("v",1)[0]`), so a versioned junction id
  (e.g. `2307.00001v2` from the URL-paste route) matches the un-versioned feed
  candidate and is correctly deduped. Regression guard:
  `tests/test_discover_for_notebook.py::TestDiscoverHappyPath::test_dedup_handles_versioned_existing_paper`.

Re-verify gate: F1 re-verified — `tests/test_notebook_api.py:261-264` confirms the
URL-paste route stores `2604.26204v3` (versioned), falsifying the synthesis's
"both sides un-versioned" assumption. No findings invalidated. Adversary
invalidation rate: 0%.
