...
Re-verified F1 (silent-Haiku for `(LOOKUP|SYNTHESIS, LEAN_WRITE)`)
and F2 (asymmetric forbidden-string check) before fixing — both
were real. All 4 MEDIUM findings fixed; F8 fixed inline as a
docs-only enhancement; F5+F6+F7+F9+F10 deferred per rectifier
protocol.

Fixed in the rectification commit:

- **F1** (MEDIUM) — fixed. Added a `_FORBIDDEN: object = object()`
  sentinel; populated `(LOOKUP, LEAN_WRITE)` and
  `(SYNTHESIS, LEAN_WRITE)` cells with the sentinel. `select_model`
  raises `ValueError` with a self-locating message naming the
  offending pair when the lookup hits the sentinel. The
  totality invariant still holds (every key is present); the
  whitelist-only invariant was extended to allow the sentinel.
  Updated `docs/model-policy.md` selection table to mark the two
  forbidden cells as **FORBIDDEN (raises `ValueError`)** with an
  explanatory paragraph.

  Regression guards: `test_f1_forbidden_cell_lookup_raises_value_error`
  (asserts the ValueError on both forbidden cells with message
  validation) and `test_f1_forbidden_cell_table_entry_is_sentinel`
  (asserts the table entry IS the sentinel via direct
  `_SELECTION_TABLE` inspection). Plus the two existing
  `test_lookup_lean_write_*` and `test_synthesis_lean_write_*`
  tests inverted from "is_haiku" to "is_forbidden" (raise
  ValueError).

- **F2** (MEDIUM) — fixed. Added
  `test_f2_haiku_and_sonnet_appear_only_in_model_selector`
  parametrized over both model ID strings. Walks `server/**/*.py`
  and asserts the Haiku and Sonnet IDs appear ONLY in
  `server/orchestrator/model_selector.py`. The Opus ban is
  already enforced by `test_no_claude_opus_in_server_python_files`;
  this F2 fix extends the SSoT property to the two used IDs. A
  future contributor hardcoding `model="claude-sonnet-4-6"` in
  any orchestrator caller will now trip CI.

- **F3** (MEDIUM) — fixed. Added `POLICY_VERSION: str = "1.0"`
  module-level constant with a docstring describing the
  cache-invalidation discipline. Added a "Cache-invalidation
  discipline" section to `docs/model-policy.md` documenting the
  three-step procedure for `_SELECTION_TABLE` changes (bump
  POLICY_VERSION + CHANGELOG row + PR description). Added a
  CHANGELOG table with the v1.0 row.

  Regression guards: `test_policy_version_pinned` (pins
  `POLICY_VERSION == "1.0"` so a bump must be a deliberate edit
  to the test), and `test_policy_doc_has_cache_invalidation_section`
  (asserts the doc section + the POLICY_VERSION mention).

- **F4** (MEDIUM) — fixed (docs). Added an "Integration status"
  paragraph to the module docstring of
  `server/orchestrator/model_selector.py` flagging that the
  module is NOT yet called by any orchestrator code (orchestrator
  dispatch lands in E08_S06+) AND that the F2 + Opus-ban tests
  provide lower-precision guards in the meantime.

- **F8** (LOW) — fixed inline. Added a "Worked example" subsection
  to `docs/model-policy.md` showing the per-query cost arithmetic
  (12 Haiku turns × $0.0045 = $0.054 for Lookup-only;
  11 Haiku + 1 Sonnet = $0.0735 ≈ $0.075 for Autoformalization).

Deferred (per rectifier protocol):

- **F5** (LOW) — `select_model` doesn't `isinstance`-check args.
  The current `ValueError` message includes `!r` of the bad input.
  Mypy catches typos at static-analysis time. Defer.
- **F6** (LOW) — brittle StrEnum-equality test. Would still fire
  for the right reason if the enum class changed. Defer.
- **F7** (LOW) — Haiku 2048-token cache caveat. Implementer
  hedged with `(Verify.)`. Defer.
- **F9** (LOW) — `_SELECTION_TABLE` correctly omitted from
  `__all__`. No fix needed.
- **F10** (LOW) — model_selector import pulls YAML loader via
  `server.router`. 17ms; not slow enough to warrant a refactor.
  Defer.

Test additions (regression guards):

- 4 in `TestRectificationGuards` in `tests/test_model_selector.py`:
  F1 forbidden raises, F1 sentinel-table-entry, F2 symmetric
  haiku/sonnet (parametrized over both IDs) — 3 distinct test
  functions.
- 2 in `TestPolicyVersion`: F3 version pin + F3 doc section.

Final test count: **46 E08_S05 tests** (was 40); full suite
**1177 passed, 4 skipped, 0 failed** (was 1171). `ruff check .`
clean.

Inner-loop attempt counts: every finding fixed in 1 attempt. F4's
docstring update accidentally re-introduced the forbidden Opus ID
string in the rephrased text — caught by the existing forbidden-
string test on first re-run; reworded to "Opus API ID" framing.
Below the 3-attempt cap.

Outer-loop iterations: 1 (single full-suite run after batched
fixes + 1 inline reword). Under the 3-iteration cap.

Invalidation rate: 0/4 MEDIUM (0%). Well below the 40% threshold.
