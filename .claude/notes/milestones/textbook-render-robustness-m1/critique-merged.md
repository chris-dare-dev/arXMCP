...
- F1 — fixed: `_sanitize_math_balance` rewritten to a stack pass that DROPS both
  orphaned closers and unclosed openers (never appends a text-mode `\end{array}`).
  Regression: `test_unclosed_begin_array_dropped`.
- F2 — fixed (pinned + documented): residue limitation noted in the docstring;
  regression `test_orphaned_closer_leaves_alignment_residue` locks the accepted
  degraded-but-renders behavior.
- F3 — fixed (pinned + documented): docstring LIMITATIONS section notes prose/heading
  token corruption; regression `test_literal_end_array_in_heading_is_dropped`.
- F4 — fixed: `_parse_latexml_timeout_from_env` docstring reworded — the server still
  REJECTS the var by design (tailored hint), matching `ARXMCP_CONTACT_EMAIL`.
- F5 — fixed: `test_latexml_timeout_env_var_rejected` added (asserts ValueError +
  message names the LaTeXML render path).
- F6 — deferred (LOW): no mutation-count observability counter. Pre-existing dead-code
  helper pattern; out of scope.
- F7 — deferred (LOW): regex matches `\begin{array}` in verbatim/code fences. Folded
  into the F3 docstring LIMITATIONS note (coarse global token scan, accepted).

Re-verify: F1 (the highest-blast MEDIUM) re-read and confirmed real (append branch at
line 155 produced a text-mode closer) before fixing. 0 findings invalidated.
