...
- **F1** — fixed in `server/routes/ui.py:250-256` via `partition("|")` reconstruction (preserves the space before the first pipe and is robust to future `compute_health_status` whitespace tweaks). Regression guards: `tests/test_status_endpoint.py::TestStatusBadge::test_badge_retrieval_warn_renders_degraded_label` and `::test_badge_ops_only_warn_renders_distinct_label` now both assert the full phrase `"DEGRADED | corpus v7 | 2 notebooks"` and `"WARN | corpus v7 | 2 notebooks"` respectively (these assertions FAIL on the pre-fix code).
- **F2** — fixed in `tests/test_status_endpoint.py::TestClassifyStatusBadge::test_warn_with_each_ops_key_renders_ops_warn` — parametrized loop over `("backup:time", "disk:utilization", "process:uptime")` symmetric to the existing retrieval-side loop. ~6 LOC, no production change.
- **F3** — fixed by F1's regression guard (full-phrase assertions added in both endpoint tests, per the adversary's "folded into F1" recommendation).
- **F4** — deferred (LOW). The implicit "unknown check → ops/WARN" default is documented in `_RETRIEVAL_CHECK_KEYS`'s docstring at `server/routes/ui.py:158-168`. A future check-key partition guard (named `_OPS_CHECK_KEYS` frozenset + introspection test) would close it; not warranted by today's check set size.

Adversary invalidation rate: 0% (3/3 CRITICAL+HIGH+MEDIUM findings re-verified as real; threshold = 40%).
