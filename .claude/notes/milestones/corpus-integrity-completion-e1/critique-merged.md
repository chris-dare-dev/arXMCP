...
- F1 — fixed in rect commit (regression guard: `tests/test_write_chunks_wap_gate.py::test_mutation_A_wrong_value_marker` strengthened with firing-point pins `"chunk_count=1"`, `"tbl.count_rows()=10"`, `"Routing: S6"` substring assertions; docstring rewritten to describe actual call-1 firing)
- F2 — fixed in rect commit (try/finally around the WAP gate so `_append_store_stats(stats)` lands regardless of gate outcome; new `WriteStats.gate_failure_reason: str` field carries the diagnostic to the audit row; regression guard: `tests/test_write_chunks_wap_gate.py::test_mutation_E_audit_row_lands_on_gate_failure_path`)
- F3 — fixed in rect commit (function-local `marker_write_failed` flag set in the swallow's except block; gate's COUNT-MISMATCH arm emits `Routing: S5 (swallow + stale marker)` or `Routing: S6 (arithmetic regression)` deterministically; runbook §Quick triage "Routing-tag shortcut" subsection documents the e1-rect-F3 contract; regression guards: `test_mutation_A_wrong_value_marker` asserts `"Routing: S6"`, `test_mutation_D_stale_marker_swallow` asserts `"Routing: S5"`)
- F4 — fixed in rect commit (two bare `assert` statements at `tests/test_server_startup_integration.py:290, 300` converted to `if not ... raise AssertionError(...)` matching the surrounding file's style; no regression guard — style fix)

**Adversary invalidation rate:** 0/4 findings invalidated by re-verify (0%). The critic's prompt calibration is clean.
