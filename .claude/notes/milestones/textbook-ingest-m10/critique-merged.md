...
All 5 findings fixed (0 invalidated, 0 deferred). Re-verify gate: all
cited file:line regions matched the findings' claims against current code.

- **F1 (MEDIUM)** — fixed in `.claude/docs/security-pdf-sandbox.md:262` (page-count snippet now `_pdf_declared_page_count(pdf_bytes: bytes)` /Count byte-regex, no PyMuPDF/Path). Regression guard: `tests/test_pdf_preflight.py::TestSecurityDocSymbolContract::test_page_count_symbol_is_declared_byte_scan_not_pymupdf`.
- **F2 (MEDIUM)** — fixed in `.claude/docs/security-pdf-sandbox.md:55` (threat-table JS row now enumerates all 7 `DANGEROUS_PDF_NAMES` tokens). Regression guard: `TestSecurityDocSymbolContract::test_dangerous_pdf_names_is_the_documented_seven_token_set`.
- **F3 (MEDIUM)** — fixed in `tests/test_pdf_preflight.py` by ADDING `TestUploadPathContentLengthGuards::test_missing_content_length_passes_middleware_to_route` (genuinely sends a chunked/no-Content-Length body and pins the benign pass-through) + tightening the class docstring. The "missing" AC case is now literally tested, not just asserted.
- **F4 (LOW)** — fixed in `.claude/docs/security-pdf-sandbox.md:55` ("m6 m8 upload's" → "m8 ar5iv upload's"); bundled into the F2 edit as recommended.
- **F5 (LOW)** — fixed in `.claude/docs/security-pdf-sandbox.md:57` (decompression-bomb row now notes RLIMIT_AS is Linux-only / non-functional on macOS, with the wall-timeout backstop).

Adversary invalidation rate: 0/5. All findings were valid and served the milestone's stated outcome (finalize the doc to match built code).
