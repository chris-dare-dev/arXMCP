...
Rect commit closes F1–F4; F5–F6 deferred. Zero findings invalidated on the Phase-4 re-verify gate.

- F1 (HIGH) — **fixed** in `server/handlers/citations.py` (`depth` Field `le=3`→`le=2`). Regression guard: `tests/security/test_resource_exhaustion.py::test_cite_neighbors_depth_3_rejected_by_validator` + `test_cite_neighbors_depth_field_constraint_present` re-pinned to `le == 2`.
- F2 (MEDIUM) — **fixed** in `server/handlers/citations.py` (the `cite_neighbors` call is wrapped in `try/except RuntimeError`, degrading to `graph_status="unavailable"` with a WARNING log). Regression guard: `tests/test_proof_chain.py::TestHandlerEndToEnd::test_handler_graph_unqueryable_returns_unavailable`.
- F3 (MEDIUM) — **fixed**: negative regression guard added — `tests/test_proof_chain.py::TestHandlerEndToEnd::test_handler_exposes_no_path_param` asserts the handler signature exposes no Kùzu/LanceDB path parameter.
- F4 (MEDIUM) — **fixed** in `server/handlers/citations.py` (`depth` default `1`→`2`, aligning the handler with the library default and the proof-chain-workflow doc's canonical `depth=2` traversal). Folded into the F1 edit.
- F5 (LOW) — **deferred**: a one-line `depends_on` caveat on the `CITE_NEIGHBORS` description. The description is not incorrect and `graph_status` already signals the not-ingested case; deferring LOW per the rectify policy.
- F6 (LOW) — **deferred**: correcting the stale `direction` enum in `.claude/notes/06-mcp-server-design.md`. Editing the design constitution is out of m1's scope; the shipped library + handler are the de-facto authority. Tracked for a future docs pass.
