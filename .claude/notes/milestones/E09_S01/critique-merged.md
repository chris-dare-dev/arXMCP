...
- F1 — fixed (CLI accepts camelCase / lowercase / uppercase via
  ``_normalize_source``; argparse usage error on unknown values).
  Regression guards: ``tests/test_graph_ingest.py::TestRectificationGuards::test_f1_source_accepts_camelcase_alias``,
  ``::test_f1_source_rejects_unknown_value``.
- F2 — fixed (added ``OPENALEX_MAX_RESPONSE_BYTES = 5 * 1024 * 1024`` and
  use it in place of the 200 MB arXiv-tarball cap). Regression guard:
  ``::test_f2_response_cap_is_tight_for_openalex_shape``.
- F3 — fixed (transient ``URLError`` writes the paper to
  ``state["fetch_failures"]``; subsequent runs drain the list; CLI
  returns exit code 1 while failures remain). Regression guards:
  ``::test_f3_fetch_failure_tracked_and_retried_on_resume``,
  ``::test_f3_cli_returns_nonzero_on_pending_failures``.
- F4 — fixed (docstring on ``_merge_paper`` flags the multi-source-write
  hazard for E09_S02; no behavioral change because INSPIRE-HEP columns
  don't exist yet — full per-field source-rank predicate is a Phase 2
  deliverable for E09_S02).
- F5 — fixed (replaced ``_read_seed_ids`` with
  ``from tools.fetch_seed import read_seed_list``). Regression guards:
  ``::test_f5_seed_reader_uses_shared_helper``,
  ``::test_f5_local_read_seed_ids_helper_removed``.
- F6 — fixed (``KUZU_SCHEMA_VERSION = 1`` constant + ``_schema_meta``
  node table stamped by ``apply_schema`` + ``read_schema_version``
  reader). Regression guards:
  ``::TestSchemaMigration::test_stamps_schema_version``,
  ``::test_read_schema_version_none_when_db_absent``.
- F7 — fixed (docstring on ``save_checkpoint`` documents the structural
  same-fs invariant; no code change needed).
- F8 — fixed (rev-map construction detects ``oa_work_id`` collisions
  and emits a warning log naming both arXiv IDs and the kept value).
  Regression guard: ``::test_f8_oa_work_id_collision_logged``.
- F9 — deferred (LOW; ``categories`` column semantic mismatch with
  arXiv categories — the column carries OpenAlex Topics display names,
  not arXiv categories. Resolution requires either a column rename
  (schema rev) or a separate arXiv-metadata fetcher; both belong in
  E09_S03+).
- F10 — fixed (dropped the ``or nested.parent.exists()`` clause from
  ``test_creates_parent_directory``; it now requires Kùzu to materialize
  the DB directory).
- F11 — deferred (LOW; ``--source`` flag's choice list is documentation
  pending E09_S02. The ``_normalize_source`` validator (F1 fix) closes
  most of this finding's surface — non-openalex values now error out
  with a clear message — but the dispatch logic for INSPIRE-HEP isn't
  wired up because INSPIRE-HEP itself isn't implemented yet).

**Invalidation rate:** 0 / 3 HIGH findings invalidated (0%). The
critic's HIGH findings all re-verified against the cited file:line
ranges; no false positives.

**Test count delta:** 1204 → 1214 passed, 4 skipped (+10 rectification
guards, +1 tightened idempotency assertion).
