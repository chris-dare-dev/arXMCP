...
2. **Filter installed on the root logger** in `configure()` so every `logging.getLogger(__name__)` in the process inherits it. The `_make_isolated_logger` test helper installs the filter on a per-test handler instead (with `propagate=False`) so pytest's session-wide log capture doesn't see the redaction side-effects — but the production install path (root logger) is covered by `TestConfigure::test_configure_installs_redaction_filter_on_root`.
