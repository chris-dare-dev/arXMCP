...
- F1 (CRITICAL) — fixed in `server/observability/logging_setup.py::configure` (now attaches `RedactionFilter` to every existing root-logger handler in addition to the root logger itself; handler-level installation is what fires on records propagated from child loggers per Python logging dispatch). Regression guard: `tests/security/test_log_redaction.py::TestConfigure::test_configure_redacts_child_logger_records_via_root_handler` (and the companion `test_configure_attaches_filter_to_handlers_added_before_configure`). End-to-end verification: a child `logging.getLogger("server.handlers.search").info(..., extra={"query": "Faltings theorem"})` now produces JSON without the `query` key.
- F2 (HIGH) — fixed by adding `test_configure_redacts_child_logger_records_via_root_handler` (exercises the production path: handler attached to root BEFORE configure() runs, configure() attaches filter to it, child logger emits and is redacted on propagation). Resolves the false-positive test result that masked F1.
- F3 (HIGH) — fixed in `.claude/notes/milestones/E13_S08/implementation-summary.md` (item 2 of "Anything notable for the critic" reworded to accurately describe what the test covers and to call out the F1 rectification). Also corrected `.claude/docs/security-observability-logging.md` § "How redaction works" to describe the handler-level installation correctly, including the post-configure handler-add contract.

**Invalidation rate:** 0% — all three findings re-verified before fix (F1 verified by direct runtime experiment that proved the root-logger-only install lets `query` leak; F2 verified by reading the test helper code that uses `propagate=False`; F3 verified by re-reading the implementation summary).
**Critic prompt health:** OK — adversary correctly identified a CRITICAL security flaw the implementer missed. Calibration is sound (CRITICAL was warranted; HIGH for both supporting findings).
