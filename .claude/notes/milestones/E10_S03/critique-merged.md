...
Re-verify ran for all 7 findings. None invalidated; every cited
file:line region matched the critique's "what" claim.

- **F1 (HIGH) — fixed.** `_element_to_node` now captures
  ``child.tail`` text as a leaf node immediately after each
  recursed child. Regression test
  `TestParseMathML::test_mixed_content_operators_distinguished`
  verifies that ``x + y``, ``x - y``, and ``xy`` produce
  structurally distinct trees with non-zero pairwise TED.
- **F2 (HIGH) — fixed.** `_local_tag` now lowercases the result
  after namespace stripping. Regression test
  `TestParseMathML::test_uppercase_tags_normalize_to_lowercase`
  asserts ``<MATH><MI>x</MI></MATH>`` and
  ``<math><mi>x</mi></math>`` produce trees at TED 0.
- **F3 (MEDIUM) — fixed.** New
  `TestHandlerDispatch::test_latex_input_returns_integral_chunk_in_top3`
  test seeds the chunks table with integral + summation chunks
  along distinct unit-vector axes, queries with the LaTeX integral
  form, asserts `retrieval_mode=="dense_only_stmt_fallback"` AND
  that the integral chunk appears in the top-3. Pins AC1's actual
  behavior on the LaTeX path.
- **F4 (MEDIUM) — fixed.** `EquationIndex.query` now tracks an
  `any_tree_seen` flag and returns ``[]`` when no candidate had a
  parseable tree, so the handler's existing
  `if not hits: dense_only_fallback` branch fires. Regression test
  `TestHandlerDispatch::test_all_null_trees_falls_through_to_dense_only_fallback`
  seeds an equation row with `mathml_tree_json=NULL` and asserts
  the resulting `retrieval_mode=="dense_only_fallback"`.
- **F5 (LOW) — fixed.** The `except (ValueError, JSONDecodeError)`
  block in `EquationIndex.query` now also catches `RecursionError`,
  degrading a pathologically-deep stored tree to cosine-only for
  that row rather than 500-ing the request. No standalone
  regression test — Python's default 1000-frame stack makes this
  hard to test deterministically; the change is defensive and
  small enough to ship without a dedicated guard.
- **F6 (LOW) — fixed.** `index_equation_trees` now builds the
  `merge_insert` Arrow table from `table.schema` (the live LanceDB
  schema) rather than the hardcoded `EQUATIONS_SCHEMA_V1`
  constant. This way a future schema bump that adds a column
  doesn't silently drop the new column on rewrite. The constant
  remains the source of truth at table-creation time.
- **F7 (LOW) — fixed.** `Resources.startup` now shares a single
  `lancedb.connect` across both the definitions table and the
  equations table blocks (replacing two distinct Database handles
  to the same directory). Imports hoisted; the per-table
  `try open_table / except ValueError` pattern stays.

**Invalidation rate:** 0 / 7 findings invalidated on re-verify
(0%). Adversary critic was well-calibrated.

**Test count delta after rectify:** 1382 passing (+4 regression
guards from post-implement 1378). 4 skipped (requires_model), 0
failed, ruff clean.

| Finding | Severity | Status |
|---|---|---|
| F1 — tail-text dropped | HIGH | fixed (regression test) |
| F2 — case mismatch | HIGH | fixed (regression test) |
| F3 — AC1 LaTeX-path test missing | MEDIUM | fixed (regression test) |
| F4 — `ted_fused` on all-NULL trees | MEDIUM | fixed (regression test) |
| F5 — unbounded recursion on stored tree | LOW | fixed (defensive) |
| F6 — indexer hard-codes schema | LOW | fixed |
| F7 — duplicate `lancedb.connect` in startup | LOW | fixed |
