...
Re-verified F1 (per-call vs per-session counter) and F2 (spoofed
session-id bypass) live before fixing — both reproduced. All HIGH
+ MEDIUM findings fixed; F11 + F12 + F13 deferred per rectifier
protocol.

Fixed in the rectification commit:

- **F1** (HIGH) — fixed (docs + regression test). Added a prominent
  ⚠️ warning block to `canonicalize_turn`'s docstring AND
  `docs/orchestrator-rules.md::When to call` documenting the
  "MUST pass FULL accumulated history each time" contract. The
  brief's "per-session monotonically increasing" wording is
  satisfied IF the orchestrator follows this contract; the
  function's idempotency makes it safe (re-canonicalizing
  already-canonical IDs is a no-op).

  Regression guards: `test_f1_growing_history_preserves_old_canonical_ids`
  (proves IDs stay stable when full history is passed across
  transitions); `test_f1_partial_history_call_collides_documented_anti_pattern`
  (proves the documented `❌ WRONG` anti-pattern actually
  collides — so a future contributor doesn't think the contract
  can be relaxed).

- **F2** (HIGH) — fixed. Added strict `^[0-9a-f]{32}$` UUID4-hex
  format check on the `mcp-session-id` header in
  `SessionCapMiddleware`. Spoofed session-ids that don't match
  the FastMCP-issued format are excluded from cap accounting; the
  request still forwards to FastMCP which will reject the unknown
  session-id with HTTP 404. An attacker could still rotate
  proper-looking UUID4 hex strings; the complete fix requires
  coupling to the FastMCP session manager's issued-id registry
  (out of scope for v1). Documented as a defense-in-depth narrowing.

  Regression guards: `test_f2_spoofed_non_hex_session_id_skips_cap`
  (5 spoofed-id requests all pass through; `get_session_count() == 0`),
  `test_f2_uppercase_hex_session_id_is_rejected_as_format`
  (uppercase hex doesn't match the lowercase regex),
  `test_f2_valid_format_session_id_engages_cap` (valid hex
  hits the cap on the 4th call).

- **F3** (MEDIUM) — fixed. Added re-export stub at
  `server/orchestrator/test_id_canon.py` that re-imports the
  test classes from `tests/test_id_canon.py`. Now
  `pytest server/orchestrator/test_id_canon.py` passes literally,
  satisfying the brief's AC #6 wording. Updated
  `docs/orchestrator-rules.md::Note on test path` to describe
  the stub.

- **F4** (MEDIUM) — fixed. `canonicalize_turn` now maps `id` and
  `tool_use_id` independently via a `_canonicalize(old_id)` closure.
  The pre-fix code used `block.get("id") or block.get("tool_use_id")`
  which short-circuited via `or` — when both fields were present
  with different values, only `id`'s value was mapped and
  `tool_use_id` was silently rewritten to that same canonical id,
  destroying the pairing.

  Regression guard: `test_f4_block_with_both_id_and_tool_use_id_maps_each_independently`
  (constructs the malformed block with both fields, asserts each
  gets a distinct canonical id, AND asserts a later `tool_result`
  reference to the original `tool_use_id` value pairs correctly).

- **F5** (LOW) — fixed inline. Hoisted the `from server.session
  import (...)` from inside `SessionCapMiddleware.__call__` to the
  top of `server/middleware.py`. Removed the misleading
  "cyclic-safe" comment.

- **F6** (MEDIUM) — fixed (test). Added two LRU eviction tests:
  `test_f6_lru_eviction_drops_oldest_when_registry_full` (patches
  `MAX_REGISTRY_SIZE = 4`, creates 5 sessions, asserts the
  oldest is evicted) and `test_f6_lru_eviction_promotes_recently_used_entry`
  (touches an entry, then verifies a subsequent eviction picks
  the truly-LRU one, not the recently-touched one).

- **F7** (MEDIUM) — fixed (test). Added
  `test_f7_middleware_order_session_cap_inside_request_body_size_limit`
  that constructs `create_app()`, walks `app.user_middleware`, and
  asserts the expected inner→outer order. SessionCapMiddleware
  MUST stay INSIDE RequestBodySizeLimitMiddleware so a malicious
  100MB request is 413-rejected before SessionCap buffers it.

- **F8** (MEDIUM) — fixed (docs). Added "Why `result.isError=true`
  rather than a JSON-RPC error envelope" subsection to
  `docs/orchestrator-rules.md`. Documents the trade-off
  explicitly: agent ergonomics over strict spec interpretation.

- **F9** (MEDIUM) — fixed. Added
  `arxmcp_retrieval_cap_rejections_total{tool}` Prometheus
  counter in `server/metrics.py`; incremented in
  `SessionCapMiddleware` on the cap-rejection path. The cap
  middleware short-circuits BEFORE FastMCP, so this counter is
  the only signal operators have for cap firing.

  Regression guard: `test_f9_cap_rejection_increments_prometheus_counter`
  drives 5 calls (3 in cap, 2 over) and asserts the counter rose.

- **F10** (LOW) — fixed inline. Added strict
  `if not isinstance(messages, list): raise TypeError(...)` at
  the top of `canonicalize_turn`. Pre-fix a tuple input would
  silently round-trip via `copy.deepcopy` and return a tuple,
  violating the documented `list[dict]` return type.

  Regression guard: `test_f10_non_list_input_raises_type_error`
  passes a tuple, a string, and `None` — all raise.

Deferred (per rectifier protocol — fix as paper-cuts in a future pass):

- **F11** (LOW) — `SessionCapMiddleware` always buffers the body for
  any POST `/mcp`, not just `tools/call`. Optimization opportunity,
  not a defect.
- **F12** (LOW) — `_retrieval_cap_payload` exposes
  `session_attempted_count`. The critic explicitly notes "acceptable
  as-is for v1" — only the requesting client's own state surfaced
  to itself, not a cross-client leak.
- **F13** (LOW) — worked example doesn't exercise nested-content
  `tool_result.content` shape. Doc enhancement; not a defect.

Test additions (regression guards):

- 4 in `TestRectificationGuards` in `tests/test_id_canon.py`:
  F1 growing-history, F1 partial-collision, F4 dual-field, F10 type
- 7 in `TestRectificationGuards` in `tests/test_session_caps.py`:
  F2 spoof×3, F6 eviction×2, F7 mount-order, F9 counter

Final test count: **53 E08_S04 tests** (was 42); full suite
**1131 passed, 4 skipped, 0 failed** (was 1120). `ruff check .`
clean.

Inner-loop attempt counts: every finding fixed in 1 attempt. F2's
fix required updating ~10 existing test session-ids to use the
UUID4-hex format (refactored via `_hex_session_id(seed)` helper)
but that was a single contiguous edit, not multiple attempts.
Below the 3-attempt cap.

Outer-loop iterations: 1 (single full-suite run after batched
fixes). Under the 3-iteration cap.

Invalidation rate: 0/2 HIGH (0%), 0/5 MEDIUM (0%) findings
invalidated. Well below the 40% threshold.
