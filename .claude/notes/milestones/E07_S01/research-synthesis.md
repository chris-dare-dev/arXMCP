...
| D9 | **`BM25Phase.query` returns `list[tuple[str, float]]` materialized.** | Matches the brief; matches the `ANNPhase` return shape E07_S02 will produce; matches `rrf.reciprocal_rank_fusion`'s expected input. |
