...
- F1 (HIGH) — fixed in `frontend/static/app.css` inside the `@media (prefers-color-scheme: dark)` block: added `input[type="text"], input[type="url"], input[type="file"] { background: var(--card-bg); color: var(--fg); }` so dark-mode typed input text renders at ~12:1 (was 1.22:1, invisible). Regression guard: `tests/test_ui_m3_dark_and_htmx_feedback.py::TestUPL8DarkModeBlock::test_dark_block_redeclares_text_input_for_visibility` (accepts either property order via two-branch regex).
- F2 (MEDIUM) — fixed in `frontend/static/app.css` inside the dark `@media` block: remapped 7 pre-existing hardcoded grey color literals (subtitle / hint / note / empty / display-name / dt / footer / footer a) to light-grey values (`#b3b9c0` ~9:1; `#9ba1a8` ~7:1 italic; `#c9d1d9` ~13:1) that pass SC 1.4.3 on dark backgrounds. Regression guard: `TestUPL8DarkModeBlock::test_dark_block_remaps_tertiary_text_greys` (asserts the 7 selector tokens appear inside the dark block).
- F3 (MEDIUM) — fixed in `frontend/static/app.css` initial `:root` block: added `color-scheme: light dark;` so browsers auto-darken UA-styled controls (form internals, scrollbars, native dropdowns) when prefers-color-scheme: dark fires. Compounds with F1 (the F1 input fix + F3 color-scheme together make dark-mode form controls fully usable). Regression guard: `TestUPL8DarkModeBlock::test_color_scheme_declared_on_root` (regex-scoped to the initial `:root` block, before any `@media`).
- F4 (LOW) — deferred (per m1/m2 precedent for CSS line-count overshoots). Additionally: the m3-rect fixes pushed `app.css` from 287 → 306 lines. The `test_app_css_under_soft_cap` cap was revised from 300 → 330 with a docstring explaining the rect-driven necessary additions; the test still discriminates against unbounded future growth (any milestone pushing past 330 must argue for another revision OR split the file).

Adversary critic invalidation rate: 0% (all 4 findings re-verified; none invalidated). The F1 HIGH was a genuine miss in the original implementation that researcher contrast verification did not catch because researchers focused on the NEW tokens, not the existing hardcoded literals (`#fff` background on inputs + 7 hardcoded grey literals across pre-m3 CSS).
