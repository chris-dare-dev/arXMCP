...
- F1 — fixed (HIGH; new test
  `test_f1_chunk_id_none_for_paper_missing_from_real_lancedb`
  builds a real LanceDB with chunks for ONLY `P_B` and asserts the
  result distinguishes "found chunk_id" from "absent paper -> None").
- F2 — addressed via documentation contract (HIGH; matches the
  project pattern from `server.corpus.open_chunks_table`'s warning).
  The `cite_neighbors` docstring carries a `.. warning::` block
  flagging that path-traversal validation is deferred to the
  E06 / E09_S04 tool-input boundary, and the MCP wrapper MUST NOT
  pass agent-supplied JSON through to `kuzudb_path` /
  `lancedb_path`. Runtime check not added because the project's
  pattern is to keep library functions trusting and lock at the
  boundary; adding one here would break `tmp_path`-based tests.
- F3 — fixed (MEDIUM; new test
  `test_f3_cited_by_depth_2_arrow_reversal` exercises depth=2
  cited_by end-to-end).
- F4 — fixed (MEDIUM; new test
  `test_f4_depends_on_depth_2_dedups_self_loop_paths` pins dedup
  on the A→A self-loop branch).
- F5 — fixed (MEDIUM; two new tests
  `test_f5_priority_list_falls_back_to_lemma` and
  `test_f5_priority_list_prefers_stmt_over_lemma`).
- F6 — fixed (MEDIUM; `_open_chunks_table_or_none` opens once per
  `ingest()` invocation; `_resolved_labels_for_paper` and
  `process_paper` now take an OPEN table handle. Two new guards:
  `test_open_chunks_table_or_none_handles_missing_path`,
  `test_none_chunks_table_returns_empty`).
- F7 — deferred (MEDIUM; `limit(1_000_000)` foot-gun. Not exercised
  in tests; saturation can't fire at Tier-3. Tier-4 milestone must
  address).
- F8 — fixed (LOW; early `ValueError` on unknown `direction`.
  Regression: `test_f8_invalid_direction_raises_early`).
- F9 — fixed (LOW; extractor filters `ltx_bibref` class. New guard:
  `TestExtractor::test_f9_excludes_ltx_bibref_class`. Existing
  `test_extracts_intra_paper_anchor_hrefs` updated — `bib1` is now
  correctly excluded since the fixture uses
  `class="ltx_ref ltx_bibref"`).
- F10 — deferred (LOW; defensive empty-rels default. Branch should
  never fire under correct Kùzu behavior. Pick up if a future Kùzu
  upgrade changes the contract).

**Invalidation rate:** 0 / 2 HIGH findings invalidated (0%). Both
HIGHs re-verified against the cited file:line ranges. F2 was
addressed via documentation rather than runtime validation —
matches the project pattern from `open_chunks_table` and the
boundary-vs-library-trust convention.

**Test count delta:** 1294 → 1302 passed, 4 skipped (+8 rectification
guards).
