...
- F1 (MEDIUM) — fixed in `.claude/docs/security-threat-model-coverage.md` (summary-table row 2 + Threat 2 section). The `server/prompts.py` reference was removed from both the table and the per-threat section; replaced with "handler-side `<retrieved_chunk>` delimiter wrapping" plus an explicit "Scope boundary" sub-section quoting CLAUDE.md §8 gotcha #6 (SYSTEM_PROMPT placeholder) and clarifying the orchestrator system-prompt instruction is out of MCP-server scope.
- F2 (LOW) — fixed in `.claude/docs/security-threat-model-coverage.md` (Gap-issue triage section). Added G7 row documenting the orchestrator system-prompt instruction as out-of-scope with severity `n/a`; added a "Scope note" paragraph explaining the gap list is for the v1 MCP server only and G7 will never be filed as an arXMCP issue.

**Invalidation rate:** 0% — both findings re-verified before fix (F1 cited line range matched the original `server/prompts.py` text exactly; F2's gap-list omission was visible at the cited region).
**Critic prompt health:** OK — both findings are doc-clarity nits with the correct MEDIUM/LOW calibration. No severity inflation. Both fixes are cheap (paragraph-scale edits, no code change, all 21 staleness tests still pass).
**Test count delta:** no new regression tests added (both findings are doc-only; the existing pytest staleness gate already covers the well-formedness contract that allows G7 with severity `n/a`).
