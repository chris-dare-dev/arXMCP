...
4 fixed (F1, F2, F3, F5), 1 accepted-deferred (F4), 0 invalidated. Re-verify
gate: the cited regions matched (the critic verified the core clean; these
are robustness/test-gap/scope-honesty findings, not "code is wrong" claims).

- **F1 (MEDIUM)** — FIXED in `tools/notebook_textbook_ingest.py`: `run()` now
  calls `validate_slug(slug)` + validates every `paper_id` via
  `is_valid_paper_id` UP FRONT (raising `NotebookError`); `main()` wraps
  `run()` in `try/except NotebookError -> exit 1` (mirrors
  `tools/notebook_ingest.py`). Explicit up-front validation, NOT a broad
  `except ValueError` (which would mask `write_chunks` invariant errors).
  Guards: `TestRunExitCodes::test_invalid_slug_clean_exit_via_main`,
  `::test_invalid_slug_validated_before_chunking`,
  `::test_invalid_paper_id_clean_exit_via_main`.
- **F2 (MEDIUM)** — FIXED via option (a): added
  `TestSearchPapersHandlerComposition::test_textbook_chunk_returned_via_handler_notebook_source_kind`
  — a textbook chunk in a notebook table, queried through the REAL
  `search_papers` handler with `filters={"notebook": slug, "source_kind":
  "textbook"}`, returns tagged `source_kind="textbook"`. Proves the full
  composition (notebook routing + source_kind threading + envelope tag) for
  a textbook chunk, closing the "closes e4 end-to-end" overclaim. Combined
  with the LanceDB-level retrieval guard + m2 routing, e4 is genuinely
  closed through the handler.
- **F3 (MEDIUM)** — FIXED:
  `TestBuildEmbedRecord::test_vector_id_alignment_not_transposed` injects a
  marker encoder (one-hot argmax derived from the chunk_id) and asserts each
  stmt row's argmax maps back to its own id — fails on any vector<->id
  transpose (which `EmbedRecord.__post_init__` provably cannot catch).
- **F5 (LOW)** — FIXED:
  `TestBuildEmbedRecord::test_real_encoder_smoke` (`@pytest.mark.requires_model`,
  skipped by default) calls `_build_embed_record` with the DEFAULT real
  `_encode_batch` and asserts an `(1, EMBEDDING_DIM)` L2-normalized array —
  pins driver<->real-encoder signature compatibility.
- **F4 (LOW)** — ACCEPTED (deferred): `--paper-id` required (not
  auto-enumerate). The implementer's rationale is sound and documented —
  `_flat_paper_id` (`:`/`/`->`_`) is not uniquely reversible, so enumerating
  `chunks/` cannot reconstruct the original paper_id. Optional future
  enhancement: enumerate from the un-flattened `papers.txt`. No code change.

Adversary invalidation rate: 0/0 HIGH+CRITICAL (none). All 5 findings valid;
the core (embed-source fidelity, FM-3 isolation, the e4 retrieval guard,
the model-free seam) was verified clean and is unchanged by this rectification.
