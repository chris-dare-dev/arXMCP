...
- **F1 | HIGH | fixed** in rect commit — replaced single-call `_seed_corpus(n=30)` with new multi-call `seed_corpus_multi_paper(n_papers=3, chunks_per_paper=10)` in `tests/_corpus_helpers.py`. THREE `write_chunks` calls with 10 chunks each; cumulative 30 rows. A regressed `chunk_count = len(chunks)` write would publish `chunk_count = 10` in the final marker against a 30-row table; the positive test's `body["chunk_count"] != _CUMULATIVE_CHUNK_COUNT` assertion catches it. The headline KR-1 regression-class protection is now actually delivered.
- **F2 | HIGH | fixed** in rect commit — test docstring + implementation summary tightened to scope coverage to `ingest.store.write_corpus_version_marker` ONLY. Sibling paths (`server/routes/notebooks._rewrite_corpus_version_marker`, `tools/notebook_reconcile_marker.py`) explicitly NOT covered. Adding a second mutation test for the notebooks route is deferred to a future epic and recorded in state.json `follow_ups`.
- **F3 | MEDIUM | fixed** in rect commit — promoted `_seed_corpus` + `_patch_model` (plus new `seed_corpus_multi_paper`) to `tests/_corpus_helpers.py`. Dropped underscores on promotion (public names: `seed_corpus`, `seed_corpus_multi_paper`, `patch_bge_m3_model`). Reconciliation test file keeps local underscore aliases via import-as. One canonical source of truth.
- **F4 | MEDIUM | fixed** in rect commit — implementation summary §AC + test file docstring rewritten to correctly describe the multi-call fixture (`_N_PAPERS` write_chunks × `_CHUNKS_PER_PAPER`) instead of the false single-call claim. F1's structural fix made the docstring drift moot.
- **F5 | MEDIUM | fixed** in rect commit — `bad_marker_writer` rewritten with `**kwargs` passthrough plus `kwargs["chunk_count"] = _CHUNKS_PER_PAPER` override. Forward-compatible with future marker-schema extensions.
- **F6 | LOW | fixed** in rect commit — dead `pytestmark = []` line dropped during the F1+F5 rewrite. Explanatory comment elevated into the file-level docstring.

**Re-verify gate:** HIGH findings re-verified pre-fix:
- F1: `tests/test_corpus_count_reconciliation.py:81` confirmed single `write_chunks(chunks, embeddings, ...)` call — adversary claim accurate.
- F2: `server/routes/notebooks.py:937` confirmed `_rewrite_corpus_version_marker` "mirrors `ingest/store.py::write_corpus_version_marker` verbatim" — adversary claim accurate.

Invalidation rate: 0/6. Single critic this milestone (infra-safety did not fire — no infra path in diff). Calibrated cleanly.

**Test count delta:** 3 → 3 tests in `tests/test_server_startup_integration.py` (same count, structurally stronger multi-call fixture); existing 40 tests in `test_corpus_count_reconciliation.py` keep passing via the shared-helper import.
