...
| an `arxmcp://formal/*` resource is registered | **LANDED** — `arxmcp://formal/{notebook}` and `/{key}`, RESOURCES not tools, so `EXPECTED_TOOL_SCHEMA_SHA256` and the BP1 prefix are untouched and `tests/test_formal_resource.py` asserts it (#175). The coverage census now carries its denominator (#179): `corpus_chunks` / `corpus_papers` / `corpus_version` / `corpus_measured_at` / `corpus_source`, read from the notebook's ingest-time `corpus-version.json` marker rather than counted live, so the number is DATED and an unauthenticated `resources/read` opens no dataset. A null denominator names its reason — `no-corpus-marker` (an answer: nothing stamped a size) is a different token from `unavailable` (a fault: a size was stamped and could not be read), and neither is `0`. No ratio is computed: entries are statements and chunks are text chunks |
