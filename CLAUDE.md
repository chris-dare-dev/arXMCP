...
**Test count (Windows 11, 2026-07-12):** 3915 passing, 99 skipped, 1 xfailed, **0 failing**; `pytest` exit 0, `ruff check .` clean. The Windows-platform failures this box used to show (≈56, up from the stale 29-failure snapshot as the suite grew) were driven to zero on 2026-07-12: **30 FIXED** and **26 GUARDED**. FIXED = test-only portability bugs (path-separator assertions normalized via `.as_posix()` / `replace("\\","/")`, missing `encoding="utf-8"` on `.tex`/`.py`/JSON reads, a `newline=""` staging write so the byte-`source_hash` is stable, a `repr()` fix for a Windows path embedded in a `python -c` string, a deterministic fake clock for a coarse-`time.monotonic()` budget test, colon-free eval-report fixture filenames) **plus one real bug in `server/routes/ui.py`** (the preview path-containment check used a hard-coded `/` separator and 404'd every valid path on Windows — replaced with `Path.is_relative_to`). GUARDED = `sys.platform == "win32"`-scoped `skipif` where the OS capability is genuinely absent: symlink creation (9), kuzu 0.11.3 mandatory-lock DB re-open (8), POSIX bash-script subprocess (5), `os.replace`-under-concurrency (1), control-char filename (1); plus 2 data-precondition skips (local curated notebooks mid-curation — a data state, NOT a portability issue). Every win32 skip still RUNS on macOS/Linux, preserving the §4.1 POSIX authority. Open follow-up: kuzu close-discipline (`del db` → explicit `conn.close(); db.close()`) for a proper cross-cutting production fix.
