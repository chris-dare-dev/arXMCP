...
_DISCLAIM_CUES = re.compile(
    ...
    r"|unverified|untested|unanswered|unresolved|unknown|open|would|whether"
    r"|forbid\w*|refus\w*|prohibit\w*|block\w*|prevent\w*|preclud\w*"
    r"|deliberately|must|may|claim\s+that|question|if)\b",
    re.IGNORECASE,
)
...
def _find_unearned_claims(text: str) -> list[str]:
    """Claim-pattern matches in sentences carrying no disclaiming cue."""
    findings: list[str] = []
    for sentence in _SENTENCE_SPLIT.split(text):
        if _DISCLAIM_CUES.search(sentence):
            continue
        for pattern in _CLAIM_PATTERNS:
            findings.extend(match.group(0) for match in pattern.finditer(sentence))
    return findings
