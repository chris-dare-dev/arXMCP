...
class TestUvRunNotInvokedFromTests:
    ...
    @staticmethod
    def _presence_check_constants(tree: ast.AST) -> set[int]:
        """Constants that are an argument to ``shutil.which`` / ``which``.

        A PATH lookup is not an invocation: it starts no process, syncs no
        environment and cannot prune a package. Banning it would push callers
        into running the binary blind to decide whether it exists, which is
        strictly worse. What stays banned is naming the binary anywhere it
        could be EXECUTED -- an argv[0], a command list, a hardcoded path.
        """
        allowed: set[int] = set()
    ...
    def test_no_uv_binary_reference_in_tests(self) -> None:
        ...
        for py in sorted(TESTS_DIR.rglob("*.py")):
            ...
            try:
                ...
                continue
            allowed = self._presence_check_constants(tree)
            for node in ast.walk(tree):
                if (
...
class TestDetectorItself:
    """The detector was narrowed to permit ``shutil.which("uv")``. That
    narrowing is only safe if it did NOT also stop catching invocation, so
    the detector is exercised against synthetic source rather than trusted.

    Without this, a future "just allow one more form" edit could hollow the
    guard out while every other test in this file still passes.
    """

    @staticmethod
    def _offenders(source: str) -> list[str]:
        tree = ast.parse(source)
        allowed = TestUvRunNotInvokedFromTests._presence_check_constants(tree)
        return [
            node.value
            for node in ast.walk(tree)
            if isinstance(node, ast.Constant)
            and isinstance(node.value, str)
            and TestUvRunNotInvokedFromTests._names_uv_binary(node.value)
            and id(node) not in allowed
        ]

    def test_presence_check_is_permitted(self) -> None:
        assert self._offenders('import shutil; shutil.which("uv")') == []
        assert self._offenders('from shutil import which; which("uv")') == []

    def test_invocation_is_still_caught(self) -> None:
        """The forms that actually sync the environment."""
        assert self._offenders('subprocess.run(["uv", "run", "python"])') == ["uv"]
        assert self._offenders('subprocess.run(["uv.exe", "sync"])') == ["uv.exe"]
        assert self._offenders('CMD = "/opt/homebrew/bin/uv"') == ["/opt/homebrew/bin/uv"]

    def test_a_which_result_that_is_then_run_is_not_laundered(self) -> None:
        """``shutil.which`` returns a PATH; passing that VARIABLE to ``run()``
        is invisible to a string-constant detector with or without the
        exemption. The exemption covers the literal argument only -- it does
        not make such a call site safe. This test records the limit rather
        than letting the guard imply cover it does not have.
        """
        src = 'uv = shutil.which("uv"); subprocess.run([uv, "run"])'
